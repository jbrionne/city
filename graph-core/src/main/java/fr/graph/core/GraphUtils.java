...
public class GraphUtils {
	...
	public static Relationship createRelationship(final Node start,
			final Node end, String lengthName, int length, String custom, String json) {
		if (start == null) {
		...
		LOG.info("createRelationshipTo " + start + " " + end);

		Relationship r = start.createRelationshipTo(end, RelTypes.EVENT);
		r.setProperty(
				Graph.NAME,
				getRelationShipName((Integer) start.getProperty(Graph.X),
						(Integer) start.getProperty(Graph.Y),
						(Integer) end.getProperty(Graph.X),
						(Integer) end.getProperty(Graph.Y)));		
		r.setProperty(lengthName, length);
		if(json != null) {
			r.setProperty(custom, json);			
		} 
		return r;
	}
	...
	public static boolean hasRelationshipWithNode(final Node start,
		...
		for (Relationship r : it) {
			LOG.info("hasRelationshipWithNode " + start + " " + r.getEndNode()
					+ " " + end + " " + r.getStartNode());
	...
	public static Node createNode(GraphDatabaseService graphDb,
		...
		if (!name2.startsWith(nodeIndex.getName())) {
			throw new IllegalArgumentException(
					"name must start with index name " + nodeIndex.getName()
							+ " " + name2);
		}
	...
	public static Node findEventNode(Index<Node> nodeIndex, String idToFind) {
		Node foundEvent = nodeIndex.get(Graph.NAME, idToFind).getSingle();
		return foundEvent;

	}

	public static Object findRelationshipByName(
			GraphDatabaseService graphDb, String indexName,
			RelationshipType typeName, String name, String property) {
		InfoRelationShip i = getRelationShipInfoByName(name);
		String nameNode = getNodeNameRoad(indexName, i.getX(), i.getZ());
		String query = "MATCH (n { " + Graph.NAME + ": '" + nameNode
				+ "' })-[r:" + typeName.name() + "]->(c) RETURN r";
		Result result = graphDb.execute(query);
		LOG.debug("findRelationshipByName " + " query " + query + " result "
				+ result.toString());
		...
		String query2 = "MATCH (n { " + Graph.NAME + ": '" + nameNode2
				+ "' })-[r:" + typeName.name() + "]->(c) RETURN r";
		Result result2 = graphDb.execute(query2);
		LOG.debug("findRelationshipByName " + " query " + query2 + " result "
				+ result2.toString());
	...
	public static List<Object> findAllRelationshipProperty(
			GraphDatabaseService graphDb, RelationshipType type, String property) {
	...
	public static Object findEventXYNodeProperty(GraphDatabaseService graphDb,
			String typeName, int x, int y, String property) {

		String query = "match (n) where n." + Graph.NAME + "=~'" + typeName
				+ ".*' AND n." + Graph.X + "=" + x + " AND n." + Graph.Y + "="
				+ y + " return n";
		Result result = graphDb.execute(query);
		LOG.debug("findEventXYNodeProperty " + " query " + query + " result "
				+ result.toString());
		while (result.hasNext()) {
			Map<String, Object> row = result.next();
	...
	public static Node findEventXYNode(GraphDatabaseService graphDb,
			String typeName, int x, int y) {
		String query = "match (n) where n." + Graph.NAME + "=~'" + typeName
				+ ".*' AND n." + Graph.X + "=" + x + " AND n." + Graph.Y + "="
				+ y + " return n";
		Result result = graphDb.execute(query);
		LOG.debug("findEventXYNode " + " query " + query + " result "
				+ result.toString());
		while (result.hasNext()) {
			Map<String, Object> row = result.next();
			for (Entry<String, Object> column : row.entrySet()) {
